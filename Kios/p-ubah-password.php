...
<body>
  ...
  $cekpassword 			= mysql_query("SELECT password FROM pemilik WHERE password='$password_lama'");
  ...
  if($_POST['submit']){
    if(mysql_num_rows($cekpassword) > 0){
          if($password_baru == $re_password_baru){
            ...
            $update 		= mysql_query("UPDATE pemilik SET password='$password_baru' WHERE id_pemilik='$id'");
