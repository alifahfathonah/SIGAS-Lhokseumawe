...
include "Get_koordinat_awal_akhir.php";  //Class untuk mendapatkan koordinat jalur angkutan umum terdekat dari posisi kita berada dan posisi kios.
include "GraphToArray.php";						 	//Class untuk menkonversi tabel ‘graph’ dari database ke dalam bentuk array[][] multi dimensional. Array ini dipergunakan untuk perhitungan algoritma dijkstra nantinya.
include "Tambah_simpul.php";					//Menkonversi koordinat jalur angkutan umum terdekat menjadi sebuah simpul baru (simpul awal dan akhir)
include "Dijkstra.php";							//perhitungan algoritma dijkstra akan di proses pada class ini.
include "Rute.php";						  	//Memproses rute yang akan dilalui
...
class Main extends GraphToArray
{
	...
	public function __construct()
	{
		...
		// DELETE TEMPORARY ID
		$k = $koneksi->connect();
		mysqli_query($this->koneksi, "DELETE FROM graph where temp = 'Y'");
	...
	*/
	public function drawRoute($shortest_path)
	{
		...
		$a = new Rute();
		$Rute_array = $a->Rute_shortest_path($exp_shortest_path, $this->old_simpul_awal, $this->old_simpul_akhir, /*tambahan-->*/ $this->maxRow0, $this->maxRow1);

		// return
		$return_json = ['jalur_shortest_path'=>$semua_latlng, 'Rute'=>$Rute_array];
